...
The content of each file is then read and stored securely inside Snowflake. The original spreadsheet itself is only ever held for the brief moment it takes to read it, and is never kept afterwards – consistent with the isolation and minimal-retention approach agreed with Antoine, nothing sensitive sits in the platform for any longer than it needs to.
