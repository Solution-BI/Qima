-- LIST_SHAREPOINT_FILES: every file under the payroll folder, one row per file.
--
-- Walks all subfolders (each subsidiary files into its own, up to 5 levels deep)
-- and returns SharePoint metadata named after the FILE_LOAD columns. Returns every
-- file, not a time window: the caller filters, and SP_INGEST_PAYROLL_FILES needs
-- the full list to detect deleted files.
--
-- Deploy with the target database and schema set as the session context.
--
-- Usage:
--   SELECT * FROM TABLE(LIST_SHAREPOINT_FILES('<tenant_id>', '<client_id>', '<drive_id>', '<folder_path>'))
--   WHERE SHAREPOINT_MODIFIED_AT > DATEADD('day', -7, CURRENT_TIMESTAMP());
...
RETURNS TABLE (
    SHAREPOINT_ITEM_ID     VARCHAR,
    FILE_NAME              VARCHAR,
    FILE_PATH              VARCHAR,
    FILE_SIZE_BYTES        NUMBER,
    SHAREPOINT_MODIFIED_AT TIMESTAMP_TZ,
    SHAREPOINT_MODIFIED_BY VARCHAR,
    SHAREPOINT_CREATED_AT  TIMESTAMP_TZ,
...
class ListSharePointFiles:
    def process(self, tenant_id, client_id, drive_id, folder_path):
        # ==========================================================================
